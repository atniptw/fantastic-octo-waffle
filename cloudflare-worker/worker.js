...
export default {
  async fetch(request, env, ctx) {
    ...
    if (
      url.pathname.match(/^\/api\/download\/[^\/]+\/[^\/]+\/[^\/]+$/) &&
      request.method === "HEAD"
    ) {
...
async function handleDownloadMetadata(env, url) {
  ...
  if (
    !isValidParam(namespace) ||
    !isValidParam(name) ||
    !isValidParam(version)
  ) {
    console.warn(
      `HEAD /api/download: Invalid parameters - ${namespace}/${name}/${version}`,
    );
  ...
  try {
    ...
    // Log success
    console.log(
      `HEAD /api/download/${namespace}/${name}/${version} → 200 ${sizeBytes} bytes`,
    );
    ...
    if (error.name === "AbortError") {
      console.warn(
        `HEAD /api/download: Timeout after 10s for ${namespace}/${name}/${version}`,
      );
...
async function handleDownloadRequest(env, url) {
  ...
  if (
    ...
    console.warn(
      `GET /api/download: Invalid parameters - ${namespace}/${name}/${version}`,
    );
    return jsonError("Invalid parameters", 400, env);
  }
  ...
  try {
    ...
    if (!response.ok) {
      const statusMsg =
        response.status === 404 ? "Mod not found" : "Download unavailable";
      const returnCode = response.status === 404 ? 404 : 502;
    ...
    console.log(
      `GET /api/download/${namespace}/${name}/${version} → 200 streaming ${contentLength} bytes`,
    );
    ...
    if (error.name === "AbortError") {
      console.warn(
        `GET /api/download: Timeout after 30s for ${namespace}/${name}/${version}`,
      );
      return jsonError("Download timeout", 504, env);
    }
