...
async function handleDownloadRequest(env, url) {
  ...
  try {
    ...
    // Handle upstream errors
    if (!response.ok) {
      const statusMsg = response.status === 404 ? 'Mod not found' : 'Download unavailable';
      const returnCode = response.status === 404 ? 404 : 502;
    ...
    console.log(`GET /api/download/${namespace}/${name}/${version} → 200 streaming ${contentLength} bytes`);
